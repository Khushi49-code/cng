...
// Define ReminderView type locally to avoid import issues
interface LocalReminderView {
  id: string;
...
const AdminDashboard: React.FC = () => {
  ...
  const filteredReminders = reminders.filter((reminder: any) => 
    reminder.days_until_expiry <= filterDays && !reminder.warranty_renewed
  );
  ...
  return (
    <div className="space-y-6 px-4 md:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="text-center">
          <div className="text-3xl font-bold text-red-600">{stats.expiringThisWeek || 0}</div>
      ...
      <Card title="Warranty Expiry Reminders">
        <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
          <h3 className="text-lg font-semibold">
            ...
            <span className="ml-2 text-sm text-gray-500">({sortedReminders.length} reminders)</span>
          </h3>

          <div className="flex space-x-2">
            <Button onClick={() => setFilterDays(7)} color={filterDays === 7 ? "blue" : "gray"} className="text-xs">7 Days</Button>
            <Button onClick={() => setFilterDays(15)} color={filterDays === 15 ? "blue" : "gray"} className="text-xs">15 Days</Button>
            <Button onClick={() => setFilterDays(30)} color={filterDays === 30 ? "blue" : "gray"} className="text-xs">30 Days</Button>
            <Button onClick={() => setFilterDays(60)} color={filterDays === 60 ? "blue" : "gray"} className="text-xs">60 Days</Button>
          </div>
        </div>
                ...
                {sortedReminders.map((reminder: any) => (
                  <tr key={reminder.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-500">{reminder.vehicle_number} | {reminder.mobile_number}</div>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">{reminder.product_name}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{formatDate(reminder.expiry_date)}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        reminder.days_until_expiry <= 0 ? "bg-red-100 text-red-800" :
                    ...
                    <td className="px-4 py-3">
                      {reminder.warranty_renewed ? (
                        <span className="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">✓ Renewed</span>
                      ) : reminder.renewal_sent ? (
                        <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">Reminder Sent</span>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      <Button
                        onClick={() => handleSendReminder(reminder.assignment_id, reminder.customer_name)}
                        color="blue"
                        className="text-xs py-1 px-2"
                        disabled={!meta.isOnline || reminder.warranty_renewed || reminder.renewal_sent}
                      >
                        {reminder.warranty_renewed ? "Renewed" : reminder.renewal_sent ? "Sent" : !meta.isOnline ? "Offline" : "Send Reminder"}
                      </Button>
                    </td>
                  </tr>
                ))}
